...
static void ProcessFace(const TopoDS_Face &face, SMesh &mesh, RgbaColor color,
    ...
    // Poly::ComputeNormals averages the facet normals within one face, so two
    // faces that meet tangentially disagree at the same point and every facet
    // boundary reads as a sharp edge. Take the normals from the surface where
    // the triangulation kept its UV nodes.
